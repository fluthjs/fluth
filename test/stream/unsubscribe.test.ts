...
describe('Stream unsubscribe scenarios', () => {
  ...
  test('test stream finish when multiple deep sync child observer', async () => {
    ...
    expect((promise$ as any)._getFlag('_cleanFlag')).toBe(true)
    expect((observable1$ as any)._getFlag('_cleanFlag')).toBe(true)
    expect((observable11$ as any)._getFlag('_cleanFlag')).toBe(true)
    expect((observable12$ as any)._getFlag('_cleanFlag')).toBe(true)
    expect((observable111$ as any)._getFlag('_cleanFlag')).toBe(true)
    expect((observable2$ as any)._getFlag('_cleanFlag')).toBe(true)
    expect((observable3$ as any)._getFlag('_cleanFlag')).toBe(true)
    expect((observable4$ as any)._getFlag('_cleanFlag')).toBe(true)
    consoleSpy.mockClear()
    promise$.next(true)
